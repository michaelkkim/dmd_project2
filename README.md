# dmd_project2
Code, posterior summaries, model diagnostics, and plots for the paper: `A novel approach to optimize composite biomarkers for continuous measures of clinical function: Application to muscle imaging biomarkers in Duchenne muscular dystrophy`

### `code1/...`
- `stan_fit_all1.R` executes Hamiltonian Monte Carlo (HMC) with No-U-Turn Sampler (NUTS) implemented in `Stan` software for our joint nonlinear mixed-effects (NLME) model of Duchenne muscular dystrophy (DMD) data (incomplete longitudinal trajectories of six-minute walk distance (6MWD) and five leg muscles' magnetic resonance imaging transverse relaxation time constant (MRI-T2) measurements) -- see Section 3.1 of paper; also contains code for `stan_results` outputs folder
